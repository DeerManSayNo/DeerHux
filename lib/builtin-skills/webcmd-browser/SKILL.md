...
cli-dependencies:
  - command: webcmd
    install-url: https://github.com/agentrhq/webcmd#manual
...
In DeerHux, check the Skill panel's CLI status first. If `webcmd` is missing,
click **下载** to install it globally. DeerHux checks CLI availability after
installation; use **重新检测** to check again when needed. The CLI requires Node.js 20.6+ and is installed
separately with `npm install -g @agentrhq/webcmd`. Follow the project's runtime
setup instructions and verify with `webcmd doctor`; finding the CLI alone does
not verify browser readiness.
